...
### `getShippingService(params: Record<string, any>)`

Retrieves a list of shipping services.

- `params`: Optional object containing filters for the request. Default includes is_enabled: true
- Returns: Promise with a list of shipping services.
- Throws: APIError if the request fails.

### `getShippingZones(params: Record<string, any> | null = null)`

Retrieves a list of shipping zones.

- `params` : Optional object containing filters for the request.
- Returns : Promise with a collection of shipping zones.
- Throws : APIError if the request fails.
